...
第 0-6 周任务已全部完成并通过本机验证。

- `npm run verify:week6` 已通过，覆盖 lint、Prisma schema、CSV 表头、CSV 完整性、第 4 周规则烟测、第 5 周推荐算法测试和第 6 周生产构建。
...
- 当前页面可以在 `http://localhost:3000` 完成安徽考生建档、分数位次校验、推荐生成、桌面表格/手机卡片展示、风险标签查看和院校专业组详情查看。
...
2. 第 0-6 周已经闭环；后续再进入第 7 周：志愿表编辑器、加入/删除、上移/下移排序、45 个志愿限制和方案保存。
