...
- 2023 一分一段、改革前本科一批/二批院校投档线，理科映射为物理类，文科映射为历史类，仅作为旧文理科院校级参考；
- 2024 一分一段、普通本科批招生计划、普通本科批专业组投档线；
- 2025 一分一段、普通本科批招生计划、普通本科批专业组投档线；
- 2025 专业录取分数线仅用于补充院校省份、城市、公私性质，不直接写入推荐历史位次。

2017-2022 数据仍保留在原始目录中，只做背景参考。2023 因为没有新高考专业组，清洗时使用合成组号 `000` 承载外键，推荐层按院校名称挂接，并在模型和页面中标注“旧文理科院校线”。
...
| 表 | 行数 |
| --- | ---: |
| `score_segments.csv` | 2,882 |
| `college_groups.csv` | 11,542 |
| `enrollment_plans.csv` | 47,401 |
| `admission_results.csv` | 10,758 |
| `colleges.csv` | 1,942 |
| `majors.csv` | 20,455 |
...
- 2026 招生计划尚未公布，当前仍以 2025 作为可报专业明细和最近一年专业组投档参考。
- 安徽院校代码存在跨年复用，且 2023 招生代码与改革后专业组代码不可直接等同；`college` 主表继续只作代码级基础信息，展示和 2023 历史匹配以 `college_groups.college_name` 年度快照为准。
